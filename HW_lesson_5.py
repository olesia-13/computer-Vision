...
for cnt in contours:
    ...
    if area > 150:
        ...
        approx = cv2.approxPolyDP(cnt, 0.03 * perimetr, True)
        if len(approx) == 3:
            shape = "trikutnik"
        elif len(approx) == 4:
            shape = "square"
        elif len(approx) > 6 and compactness > 0.7:
            shape = "oval"
        else:
            shape = "inshe"
        ...
        cv2.putText(img_copy, f'S: {int(area)}, P: {int(perimetr)}', (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 0.9, (255, 0, 0), 2)
