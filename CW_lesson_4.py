...
img = cv2.imread("images/girly.jpg")
scale = 1
...
img_copy = cv2.Canny(img_copy, 50, 130)

contours, hierarchy = cv2.findContours(img_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
# ретр - знаходить лише крайні зовнішні контури - останні. чеін - ключові точки, по яких визначаємо контур

# Малювання контурів прямокутників та тексту
for cnt in contours:
    area = cv2.contourArea(cnt)
    if area > 80: # фільтр шуму
